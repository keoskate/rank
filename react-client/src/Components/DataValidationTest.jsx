  ...
  const runTest = async () => {
    ...
    for (const symbol of TEST_SYMBOLS) {
      ...
      try {
        ...
        const yearHighPct = ((yearHighDiff / oldData.yearHigh) * 100).toFixed(
          2
        );
    ...
    <div style={{ padding: '40px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      >
        ...
        <p style={{ margin: 0, color: '#6c757d', fontSize: '16px' }}>
          Multi-source validation system test - comparing old vs new data
          quality
        </p>
      </div>
        ...
        <div>
          {testResults.map((result, index) => (
            ...
            >
              ...
              >
                <h2 style={{ margin: 0, color: '#2c3e50' }}>{result.symbol}</h2>
                <div
                  style={{
                    padding: '8px 16px',
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: '14px', lineHeight: '1.8' }}>
                    <div>
                      <strong>Price:</strong> $
                      {result.oldData.price?.toFixed(2)}
                    </div>
                    <div>
                      <strong>52W High:</strong> $
                      {result.oldData.yearHigh?.toFixed(2)}
                      {result.oldData._dataQuality?.metrics?.yearHigh ===
                        'estimated' && (
                    ...
                    <div>
                      ...
                      {(result.oldData.marketCap / 1000000000).toFixed(2)}B
                    </div>
                    <div>
                      <strong>P/E Ratio:</strong>{' '}
                      {result.oldData.peRatio?.toFixed(2)}
                      {result.oldData._dataQuality?.metrics?.peRatio ===
                        'estimated' && (
                        <span style={{ color: '#dc3545', marginLeft: '8px' }}>
                          ⚠️ ESTIMATED
                        </span>
                    ...
                    <div>
                      <strong>Beta:</strong> {result.oldData.beta?.toFixed(2)}
                      {result.oldData._dataQuality?.metrics?.beta ===
                        'estimated' && (
                        <span style={{ color: '#dc3545', marginLeft: '8px' }}>
                          ⚠️ ESTIMATED
                        </span>
                ...
                >
                  ...
                  <div style={{ fontSize: '14px', lineHeight: '1.8' }}>
                    <div>
                      <strong>Price:</strong> $
                      {result.newData.price?.toFixed(2)}
                      <span
                        style={{
                          color: '#28a745',
                          marginLeft: '8px',
                    ...
                    <div>
                      <strong>52W High:</strong> $
                      {result.newData.yearHigh?.toFixed(2)}
                      <span
                        style={{
                          color: '#28a745',
                          marginLeft: '8px',
                    ...
                    <div>
                      <strong>Market Cap:</strong> $
                      {(result.newData.marketCap / 1000000000).toFixed(2)}B
                      <span
                        style={{
                          color: '#28a745',
                          marginLeft: '8px',
                    ...
                    <div>
                      <strong>P/E Ratio:</strong>{' '}
                      {result.newData.peRatio?.toFixed(2)}
                      <span
                        style={{
                          color: '#28a745',
                          marginLeft: '8px',
                    ...
                    <div>
                      <strong>Beta:</strong> {result.newData.beta?.toFixed(2)}
                      <span
                        style={{
                          color: '#28a745',
                          marginLeft: '8px',
              ...
              <div
                style={{
                  backgroundColor:
                    Math.abs(result.yearHighPct) > 10 ? '#fff3cd' : '#d4edda',
                ...
                <div style={{ fontSize: '15px', lineHeight: '1.8' }}>
                  {Math.abs(result.yearHighPct) > 10 ? (
                    <div style={{ color: '#856404', fontWeight: '600' }}>
                      🚨 52-Week High was{' '}
                      <strong>
                        {Math.abs(result.yearHighPct).toFixed(1)}% WRONG!
                      </strong>
                    </div>
                  ) : Math.abs(result.yearHighPct) > 5 ? (
                    <div style={{ color: '#856404' }}>
                      ⚠️ 52-Week High had{' '}
                      {Math.abs(result.yearHighPct).toFixed(1)}% deviation
                    </div>
                  ) : (
                    <div style={{ color: '#155724' }}>
                      ✅ 52-Week High was reasonably accurate (
                      {Math.abs(result.yearHighPct).toFixed(1)}% difference)
                    </div>
                  )}

                  <div style={{ marginTop: '10px' }}>
                    <strong>Validation Status:</strong> {result.verifiedMetrics}
                    /{result.totalMetrics} metrics verified across multiple
                    sources
                  </div>
                  ...
                  >
                    Data sources:{' '}
                    {result.newData._validation?.metrics?.yearHigh?.sources?.join(
                      ', '
                    ) || 'N/A'}
                  </div>
                </div>
      ...
      {testResults.length > 0 && (
        ...
        >
          ...
          <p style={{ margin: 0, fontSize: '16px', color: '#6c757d' }}>
            Validated {testResults.length} stocks. Check the console for
            detailed logs.
          </p>
        </div>
      )}
