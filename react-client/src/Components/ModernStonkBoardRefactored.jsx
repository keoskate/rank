...
function ModernStonkBoardRefactored() {
    ...
    const { 
        ...
        initializeData, 
        setData 
    } = useStockData(config.ACTIVE_STOCKS, config.DEBUG_MODE);
    ...
    useEffect(() => {
        ...
        initialize();
    }, [initializeData, params, config.LOADING.INITIAL_BATCH_SIZE, config.LOADING.FETCH_FINANCIALS]);
     ...
     */
    const getCellStyleMemo = useCallback((col, value) => {
        ...
        return (
            <div style={{ 
                ...
                <div style={{ fontSize: '14px', marginTop: '10px' }}>
                    {config.DEBUG_MODE ? 'Using cached data' : 'Fetching live data from API'}
                </div>
            </div>
        );
    }
            ...
            <div style={{ marginTop: 20, fontSize: '12px', color: '#666' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        ...
                        Mode: <strong>{config.DEBUG_MODE ? 'Debug' : 'Live'}</strong>
                    </div>
                    <div>
                ...
                <div style={{ marginTop: '5px', fontSize: '11px', color: '#999' }}>
                    Rankings calculated using dual algorithms: relative position + statistical deviation analysis
                </div>
            </div>
        </div>
    );
