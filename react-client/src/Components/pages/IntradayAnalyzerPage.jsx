...
const IntradayAnalyzerPage = () => {
  ...
  const closePosition = async (takeProfits = false) => {
    ...
    if (
      !confirm(
        `Are you sure you want to ${action} for ${symbol}? This will sell ${Math.abs(currentPosition.qty)} shares at market price.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        alert(
          `Position closed successfully! Sold ${Math.abs(currentPosition.qty)} shares.`
        );
        setCurrentPosition(null);
        setTradeResult(null);
      } else {
  ...
  const executeTrade = async () => {
    ...
    const estimatedEntry =
      strategyResults.optimalStrategy.metrics.avgEntryPrice || 0;
    ...
    try {
      ...
      if (data.success && data.shouldEnter) {
        alert(
          `Trade executed successfully! Buy order placed for ${data.quantity} shares at market price with $${profitTargetDollars} profit target.`
        );
    ...
    >
      ...
      <div style={{ marginBottom: theme.spacing.xl }}>
        ...
        <p style={{ margin: 0, color: theme.colors.textMuted }}>
          Day trading analysis with real-time market sentiment and entry/exit
          recommendations
        </p>
      </div>
      ...
      {currentPosition && (
        <Card
          variant={
            parseFloat(currentPosition.unrealized_pl) >= 0 ? 'success' : 'error'
          }
          padding="large"
      ...
      {analysis && !loading && (
        <div>
          ...
          >
            ...
            <div>
              ...
              <Card padding="medium" style={{ marginBottom: theme.spacing.lg }}>
                ...
                >
                  5-Minute Candles ({analysis.intraday.candles?.length || 0}{' '}
                  bars)
                </h3>
                  ...
                  <div
                    style={{
                      ...
                      color: getPatternColor(
                        analysis.intraday.analysis?.pattern
                      ),
                    }}
                  >
                ...
                <Card padding="medium">
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {analysis.intraday.swingAnalysis.change30min
                          ? `${analysis.intraday.swingAnalysis.change30min > 0 ? '+' : ''}${analysis.intraday.swingAnalysis.change30min}%`
                          : 'N/A'}
                      </div>
                    </div>
                    <div
                    ...
                    >
                      ...
                      >
                        {analysis.intraday.swingAnalysis.change3hr
                          ? `${analysis.intraday.swingAnalysis.change3hr > 0 ? '+' : ''}${analysis.intraday.swingAnalysis.change3hr}%`
                          : 'N/A'}
                      </div>
                    </div>
                    <div
                    ...
                    >
                      ...
                      >
                        {analysis.intraday.swingAnalysis.changeClose
                          ? `${analysis.intraday.swingAnalysis.changeClose > 0 ? '+' : ''}${analysis.intraday.swingAnalysis.changeClose}%`
                          : 'N/A'}
                      </div>
                    </div>
                  </div>
            ...
            <div>
              ...
              <Card
                variant={
                  analysis.recommendations?.action?.includes('BUY')
                    ? 'success'
                    : analysis.recommendations?.action?.includes('SELL')
                      ? 'error'
                      : 'warning'
                }
                padding="medium"
          ...
          {analysis.technicals && (
            <Card padding="medium" style={{ marginBottom: theme.spacing.lg }}>
              ...
              >
                <TechnicalIndicator
                  label="RSI (14)"
                  value={analysis.technicals.rsi?.toFixed(2)}
                />
                <TechnicalIndicator
                  label="SMA 20"
                  value={`$${analysis.technicals.sma20?.toFixed(2) || 'N/A'}`}
                />
                <TechnicalIndicator
                  label="SMA 50"
                  value={`$${analysis.technicals.sma50?.toFixed(2) || 'N/A'}`}
                />
                <TechnicalIndicator
                  label="SMA 200"
                  value={`$${analysis.technicals.sma200?.toFixed(2) || 'N/A'}`}
                />
                <TechnicalIndicator
                  label="Volatility"
                  value={`${analysis.technicals.volatility?.toFixed(2) || 'N/A'}%`}
                />
              </div>
            </Card>
          )}
            ...
            <Card padding="medium" style={{ marginBottom: theme.spacing.lg }}>
              ...
              >
                {stockDetails.currentPrice && (
                  <TechnicalIndicator
                    label="Current Price"
                    value={`$${parseFloat(stockDetails.currentPrice).toFixed(2)}`}
                  />
                )}
                {stockDetails.technicals?.rsiSignal && (
                  <TechnicalIndicator
                    label="RSI Signal"
                    value={stockDetails.technicals.rsiSignal}
                  />
                )}
                {stockDetails.technicals?.trendSignal && (
                  <TechnicalIndicator
                    label="Trend Signal"
                    value={stockDetails.technicals.trendSignal}
                  />
                )}
                {stockDetails.technicals?.priceChange1D && (
                  <TechnicalIndicator
                    label="1D Change"
                    value={`${stockDetails.technicals.priceChange1D}%`}
                  />
                )}
                {stockDetails.technicals?.priceChange1W && (
                  <TechnicalIndicator
                    label="1W Change"
                    value={`${stockDetails.technicals.priceChange1W}%`}
                  />
                )}
                {stockDetails.technicals?.priceChange1M && (
                  <TechnicalIndicator
                    label="1M Change"
                    value={`${stockDetails.technicals.priceChange1M}%`}
                  />
                )}
                {stockDetails.technicals?.distanceFromHigh && (
                  <TechnicalIndicator
                    label="From 52W High"
                    value={`${stockDetails.technicals.distanceFromHigh}%`}
                  />
                )}
                {stockDetails.recommendation && (
                  <MetricCard
                    ...
                    variant={
                      stockDetails.recommendation === 'Strong Buy' ||
                      stockDetails.recommendation === 'Buy'
                        ? 'success'
                        : stockDetails.recommendation === 'Sell'
                          ? 'error'
                          : 'warning'
                    }
                  />
          ...
          {analysis.marketSentiment &&
            analysis.intraday.candles &&
            analysis.intraday.candles.length > 0 && (
            ...
            {showStrategyOptimizer && (
              <div>
                ...
                >
                  ...
                  >
                    <strong>💡 How it works:</strong> Analyzes the first 3 hours
                    of trading (9:30 AM - 12:30 PM ET) along with SPY and VIX to
                    determine if we should enter. If conditions are met, enters
                    at 12:30 PM and exits when hitting profit target (default
                    10%) or at market close. Tests various momentum thresholds
                      ...
                      >
                        ...
                        >
                          <div style={{ flex: '0 0 auto' }}>
                            ...
                            <input
                              ...
                              onChange={e =>
                                setProfitTargetDollars(Number(e.target.value))
                              }
                              min="100"
                              step="100"
                              style={{
                          ...
                          <div style={{ flex: '0 0 auto', paddingTop: '22px' }}>
                            ...
                            >
                              {tradeExecuting
                                ? 'Executing...'
                                : '🔴 Try Live Trade (Market Hours Only)'}
                            </Button>
                          </div>
                        </div>
                        ...
                        >
                          <div>
                            ...
                            >
                              {
                                strategyResults.optimalStrategy.strategy
                                  .minMomentum3Hr
                              }
                              %
                            </div>
                          ...
                          <div>
                            ...
                            >
                              {
                                strategyResults.optimalStrategy.metrics
                                  .profitFactor
                              }
                            </div>
                          </div>
                        ...
                        {strategyResults.optimalStrategy.metrics
                          ...
                          >
                            ...
                            >
                              📊 Historical Backtest Results (From Date Range
                              Above)
                            </h5>
                            <div
                              style={{
                            ...
                            >
                              These are average prices from winning trades in
                              the backtested period. NOT live market prices.
                            </div>
                            <div
                              style={{
                                display: 'grid',
                        ...
                        >
                          ...
                          >
                            This strategy now includes{' '}
                            <strong>ranking list stocks</strong> as additional
                            market indicators:
                            <br />
                            <ul
                              style={{
                            ...
                            >
                              <li>
                                Analyzes 10 stocks from the COVID_19 ranking
                                list for broader market sentiment
                              </li>
                              <li>
                                Requires at least{' '}
                                <strong>
                              ...
                              <li>
                                Filters out days when overall market average is
                                declining {'>'} -1.0%
                              </li>
                              <li>
                                Provides more robust entry signals by
                                correlating with market breadth
                              </li>
                            </ul>
                          </div>
                        </div>
                        ...
                        {strategyResults.optimalStrategy.dailyLogs &&
                          strategyResults.optimalStrategy.dailyLogs.length >
                            0 && (
                            <div
                              style={{
                                padding: '15px',
                            ...
                            >
                              ...
                              >
                                How the algorithm performed on each trading day
                              ...
                              >
                                {strategyResults.optimalStrategy.dailyLogs
                                  ...
                                  .map((log, idx) => {
                                    ...
                                    const isMissed =
                                      log.status === 'no_signal' &&
                                      log.missedOpportunity;
                                    const isNoSignal =
                                      log.status === 'no_signal' &&
                                      !log.missedOpportunity;
                                    const isError =
                                              ...
                                              <span>
                                                ...
                                                >
                                                  ...
                                                  {log.profitPercent > 0
                                                    ? '+'
                                                    : ''}
                                                  {log.profitPercent.toFixed(2)}
                                                  %)
                                                </strong>
                                              </span>
                                              <span>
                                                Momentum:{' '}
                                                <strong>{log.momentum}%</strong>
                                              </span>
                                            </div>
                                            {log.reason && (
                                              <div
                        ...
                        {strategyResults.optimalStrategy.metrics
                          ...
                          >
                            ...
                            >
                              🤖 Automated Order Template (for future API
                              integration)
                            </h5>
                            <div
                              style={{
                            ...
                            >
                              <div style={{ marginBottom: '8px' }}>
                                <strong>
                                  1. Entry Order (at 12:30 PM ET if conditions
                                  met):
                                </strong>
                              </div>
                              <div
                              ...
                              >
                                ...
                                % in first 3hrs
                                <br />• SPY: {'>'} -0.5% in first 3hrs
                                <br />• VIX: {'<'} +5% in first 3hrs
                                <br />• Market Breadth: {'>'}={' '}
                                {strategyResults.optimalStrategy.strategy
                                  .minMarketBreadth || 40}
                                % of ranking stocks positive
                                <br />• Overall Market: Avg ranking stocks {
                                  '>'
                                }{' '}
                                -1.0%
                              </div>
                              ...
                              >
                                Type: <strong>LIMIT SELL</strong>
                                <br />
                                Symbol: {symbol}
                                <br />
                                Limit Price: Entry Price ×{' '}
                                {(
                                  1 +
                                  strategyResults.optimalStrategy.strategy
                                    .profitTarget /
                              ...
                              <div style={{ marginBottom: '8px' }}>
                                <strong>
                                  3. Backup Exit Order (market close):
                                </strong>
                              </div>
                              <div
                                style={{
                              ...
                              >
                                Type: <strong>MARKET ON CLOSE (MOC)</strong>
                                <br />
                                Symbol: {symbol}
                                <br />
                                Time: 3:50 PM ET (10 min before close)
                                <br />
...
const ProfitCalculator = ({ entryPrice, exitPrice, stopLoss, symbol }) => {
  ...
  const riskRewardRatio = (Math.abs(totalProfit) / Math.abs(totalLoss)).toFixed(
    2
  );
  ...
  return (
    <Card padding="medium" style={{ marginBottom: theme.spacing.lg }}>
      ...
      >
        <strong>Example Trade:</strong> Buy {shares.toLocaleString()} shares of{' '}
        {symbol} at ${entryPrice.toFixed(2)}, sell at ${exitPrice.toFixed(2)}{' '}
        (target) or ${stopLoss.toFixed(2)} (stop-loss). Expected profit if
        target is hit:{' '}
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        {[0, 0.25, 0.5, 0.75, 1].map(ratio => {
          ...
          const price =
            minPrice -
            pricePadding +
            (1 - ratio) * (priceRange + 2 * pricePadding);
          return (
            <g key={ratio}>
              <line
                x1={padding.left}
      ...
      {hoveredCandle && (
        ...
        >
          <div
            style={{ fontWeight: '600', marginBottom: '6px', color: '#e9ecef' }}
          >
...
const IntradayCorrelationChart = ({
  stockSymbol,
  stockCandles,
  marketSentiment,
}) => {
  ...
  const metricsData = useMemo(() => {
    ...
    if (
      marketSentiment?.spyCandles &&
      marketSentiment.spyCandles.length === stockCandles.length
    ) {
      result.SPY = marketSentiment.spyCandles.map(c => c.close);
    }

    if (
      marketSentiment?.vixCandles &&
      marketSentiment.vixCandles.length === stockCandles.length
    ) {
  ...
  if (Object.keys(metricsData).length <= 1) {
    return (
      ...
      >
        ...
        >
          Market correlation data (SPY, VIX) not available for this date.
          Showing stock price only.
        </div>
      </Card>
    );
  }
  ...
  return (
    <Card padding="medium" style={{ marginBottom: theme.spacing.lg }}>
      ...
      >
        <strong>💡 Tip:</strong> Strong positive correlation with SPY suggests
        the stock is following broader market trends. Strong negative
        correlation with VIX (fear index) is typical for most stocks during calm
        markets. Weak correlations may indicate stock-specific news or events.
      </div>
    </Card>
  );
};
