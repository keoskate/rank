...
const PaperTradingPage = () => {
  ...
  const switchTradingMode = async newMode => {
    if (tradingMode.isLive && newMode === 'paper') {
      if (
        !confirm(
          'Switch from LIVE to PAPER mode? Future orders will use paper trading.'
        )
      ) {
        return;
      }
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      if (data.success) {
        alert(
          `✅ ${orderType.toUpperCase()} order placed: ${quantity} shares of ${selectedSymbol}\nOrder ID: ${data.order.id}\nStatus: ${data.order.status}`
        );
        setQuantity(10);
        setSelectedSymbol('');
        await loadAllData();
  ...
  const closeAllPositions = async () => {
    if (
      !confirm(
        '⚠️ This will CLOSE ALL POSITIONS and cancel all orders. Are you sure?'
      )
    ) {
      return;
    }
    ...
    try {
      ...
      alert(
        'Close all positions functionality - this would liquidate everything'
      );
        ...
        >
          ...
          >
            <div>
              ...
              >
                <strong>Account:</strong> {tradingMode.accountNumber} •{' '}
                <strong>Mode:</strong> {tradingMode.mode.toUpperCase()}
              </p>
            </div>
          ...
          {tradingMode.isLive && tradingMode.safetyConfig && (
            ...
            >
              ...
              >
                Max Order: $
                {tradingMode.safetyConfig.maxOrderValue.toLocaleString()} • Max
                Daily Trades: {tradingMode.safetyConfig.maxDailyTrades} •
                Confirmation Required:{' '}
                {tradingMode.safetyConfig.requireDoubleConfirm ? 'YES' : 'NO'}
              </p>
            </div>
          )}
        </Card>
      ...
      {account && (
        ...
        >
          ...
          >
            ...
            <div>
              <strong>Pattern Day Trader:</strong>
              <br />
              {account.pattern_day_trader ? 'Yes' : 'No'}
            </div>
            <div>
              <strong>Daytrade Count:</strong>
              <br />
            ...
            >
              ...
              {rankings.map((stock, idx) => (
                <option key={stock.ticker} value={stock.ticker}>
                  {stock.ticker} - ${stock.price?.toFixed(2) || 'N/A'} (Rank #
                  {idx + 1})
                </option>
              ))}
            </select>
        ...
        <Card padding="large">
          ...
          <div style={{ marginBottom: theme.spacing.md }}>
            ...
            <input
              ...
              onChange={e =>
                setAutoConfig({ ...autoConfig, topN: parseInt(e.target.value) })
              }
              min="1"
              max="20"
              style={{
          ...
          <div style={{ marginBottom: theme.spacing.lg }}>
            ...
            >
              {autoConfig.positionSize}% of portfolio per stock (~$
              {account
                ? (
                    parseFloat(account.portfolio_value) *
                    (autoConfig.positionSize / 100)
                  ).toLocaleString()
                : '0'}{' '}
              each)
            </small>
          ...
          >
            {autoTrading
              ? '⏳ Executing...'
              : `🚀 Buy Top ${autoConfig.topN} Stocks`}
          </Button>
          ...
          >
            <span style={{ color: theme.colors.infoDark }}>
              <strong>Auto-Trade Preview:</strong> Will buy equal $ amounts of
              top {autoConfig.topN} stocks using {autoConfig.positionSize}%
              position sizing.
            </span>
          </Card>
        </Card>
              ...
              <tbody>
                {positions.map((pos, index) => (
                  ...
                  >
                    ...
                    >
                      {pos.unrealizedPL >= 0 ? '+' : ''}$
                      {pos.unrealizedPL.toLocaleString('en-US', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </td>
