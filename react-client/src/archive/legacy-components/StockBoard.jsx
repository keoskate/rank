...
class StockBoard extends Component {
  ...
  async getFinancialData(stocks) {
    const endPoint =
      'https://apidojo-yahoo-finance-v1.p.rapidapi.com/stock/get-detail?region=US&lang=en&symbol=AAPL';
    ...
    try {
      ...
      const data = this.parseData(
        Object.assign(
          ...
          results.length === 2 ? await results[1].json() : {}
        )
      );
  ...
  calculateRank(data, rGrid, sGrid) {
    ...
    data.map((row, index) => {
      ...
      const count =
        self.countIf(range, rank) === 0 ? 1 : self.countIf(range, rank) + 1;
  ...
  revertSortFunc(a, b, order, sortField) {
  ...
  handleWeightChange(evt) {
    if (
      ...
      this.state.params[evt.target.name].weight < evt.target.valueAsNumber
    ) {
      return;
    }
  ...
  handleMultiplierClick(evt) {
    this.state.params[evt.target.name].multiplier =
      this.state.params[evt.target.name].multiplier === 1 ? -1 : 1;
  ...
  parseData(data) {
    ...
    const formatData = {
      ...
      quickRatio:
        Number.parseFloat(data.financialData.quickRatio.raw || 0).toFixed(2) ||
        0,
  ...
  cleanRow(item) {
    return {
      rank: 0,
      marketCap:
        item.marketCap === 0 ? 0 : Number(item.marketCap.replace(/,/g, '')),
      ...
      discount:
        item.discount === 0 ? 0 : Number(item.discount.replace(/%/, '')),
      distribution:
        item.distribution === 0
          ? 0
          : Number(item.distribution.replace(/%/, '')),
      leverage:
        item.leverage === 0 ? 0 : Number(item.leverage.replace(/%/, '')),
      uniiDist:
        item.uniiDist === 0 ? 0 : Number(item.uniiDist.replace(/%/, '')),
    };
  }
        ...
        return (
          <span style={{ display: 'inline-block', margin: 5, width: 120 }}>
            <WeightSlider
              label={`${key}`}
              name={key}
              value={this.state.params[key].weight}
      ...
      if (key === 'rank') {
        ...
        return (
          <TableHeaderColumn
            ...
            sortFunc={this.revertSortFunc}
          >
            <div style={{ 'white-space': 'pre-line' }}>{params[key].label}</div>
          </TableHeaderColumn>
        );
      }
    });
