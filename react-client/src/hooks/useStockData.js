...
export const useStockData = (stocksConfig, debugMode = true) => {
    ...
    const getFinancialData = useCallback(async (stocks, fetchFinancials = false) => {
    ...
    const fetchAllData = useCallback(async (offset, getFinancials = false, onProgressUpdate) => {
        wait(THROTTLE.LARGE);

        for (let i = offset; i < stocksConfig[0].length; i += offset) {
            try {
                const stockData = await getFinancialData(stocksConfig[0].slice(i, i + offset), getFinancials);
    ...
    const initializeData = useCallback(async (params, offset = 5, getFinancials = false) => {
        ...
        try {
            ...
            const initialData = await getFinancialData(stocksConfig[0].slice(0, offset), getFinancials);
            const cleanedData = debugMode ? initialData : cleanData(initialData, params);
