...
export function cacheStockData(key, data, metadata = {}) {
  try {
    ...
    if (serialized.length > CACHE_CONFIG.maxCacheSize) {
      console.warn(`❌ Cache entry too large for ${key} (${serialized.length} bytes)`);
      return false;
    }
    
    localStorage.setItem(cacheKey, serialized);
    console.info(`✅ Successfully cached ${key}: ${data.length} stocks, expires in ${formatDuration(CACHE_CONFIG.maxAge)}`);
...
export function getCachedStockData(key) {
  try {
    ...
    console.info(`📦 Using cached ${key}: ${cacheEntry.data.length} stocks, age: ${formatDuration(age)}`);
...
function validateStockData(data) {
  ...
  if (data.length < CACHE_CONFIG.minValidStocks) {
    console.warn(`❌ Insufficient stocks: ${data.length} < ${CACHE_CONFIG.minValidStocks}`);
    return false;
  }
  
  // Check if at least 80% of stocks have required fields
  const validStocks = data.filter(stock => {
    return CACHE_CONFIG.requiredFields.every(field => 
      stock && typeof stock === 'object' && stock[field] != null
    );
  });
  ...
  if (validPercentage < 0.8) {
    console.warn(`❌ Too many invalid stocks: ${validStocks.length}/${data.length} (${Math.round(validPercentage * 100)}%)`);
    return false;
  }
  
  console.info(`✅ Data validation passed: ${validStocks.length}/${data.length} valid stocks`);
  return true;
}
  ...
  try {
    ...
    if (staleCache) {
      try {
        ...
        console.warn(`⚠️ Using stale cache for ${cacheKey} due to fetch failure`);
        return cacheEntry.data;
      } catch (parseError) {
 ...
 */
export async function cacheWithBackgroundRefresh(cacheKey, fetchFunction, updateCallback, options = {}) {
  const { maxAge = CACHE_CONFIG.maxAge, provider = 'api', forceRefresh = false } = options;
  ...
  if (cached && !forceRefresh) {
    try {
      const cacheEntry = JSON.parse(localStorage.getItem(CACHE_PREFIX + cacheKey));
      const age = Date.now() - cacheEntry.metadata.cachedAt;
      const isStale = age > (30 * 60 * 1000); // 30 minutes
      
      if (isStale) {
        console.info(`🔄 Cache is ${Math.round(age / 60000)} minutes old, refreshing in background...`);
  ...
  if (shouldFetchFresh) {
    ...
    fetchFunction()
      .then(freshData => {
        if (validateStockData(freshData)) {
          console.info(`✅ Background fetch complete for ${cacheKey}, updating cache...`);
