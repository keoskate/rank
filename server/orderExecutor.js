...
async function executeEntry(sessionId, symbol, decision) {
  ...
  if (session.config.simulationMode) {
    return simulatedExecutor.simulatedEntry(session, symbol, decision);
  }
  ...
  try {
      ...
      try {
        ...
        if (!pdtStatus.canDayTrade) {
          tradingLogger.logRisk('PDT LIMIT REACHED', {
            ...
            action:
              'Blocking day trade entry - position would need overnight hold',
          });
        ...
        tradingLogger.logError('[AI Engine] PDT check failed', {
          sessionId,
          sessionName: session.name,
          symbol,
          error: pdtError.message,
        });
      }
    }

    // Calculate position size (guard against NaN from corrupted position data)
    const positionsMarketValue = Array.from(
      session.portfolio.positions.values()
    ).reduce((sum, p) => sum + (parseFloat(p.marketValue) || 0), 0);
    const portfolioValue =
      (parseFloat(session.portfolio.cash) || 0) + positionsMarketValue;
    ...
    if (!effectivePortfolioValue || effectivePortfolioValue < 1000) {
      ...
      try {
        ...
        effectivePortfolioValue =
          ...
          session.config.allocatedCapital ||
          100000;
        ...
        tradingLogger.logRisk('Using default portfolio value', {
          sessionId,
          sessionName: session.name,
          reason: 'Account fetch failed',
          value: effectivePortfolioValue,
          action: `Using $${effectivePortfolioValue.toLocaleString()} default`,
        });
      }
    }
    ...
    if (
      ...
      tradingLogger.logRisk('Portfolio value capped', {
        sessionId,
        sessionName: session.name,
        symbol,
        reason: `Capped by allocatedCapital: $${effectivePortfolioValue.toFixed(0)} -> $${allocatedCap}`,
        value: effectivePortfolioValue,
        threshold: allocatedCap,
      });
      effectivePortfolioValue = allocatedCap;
    }
    ...
    const effectiveMaxPercent = Math.min(
      sessionMaxPercent,
      ctx.GLOBAL_MAX_POSITION_PERCENT
    );
    ...
    const confidenceScale = Math.min(
      Math.max((entryConfidence - 60) / 30, 0),
      1.0
    );
    const basePositionPercent = 8;
    const scaledMaxPercent =
      basePositionPercent +
      (effectiveMaxPercent - basePositionPercent) * confidenceScale;
    let maxPositionValue = effectivePortfolioValue * (scaledMaxPercent / 100);

    tradingLogger.logInfo(
      `[AI Engine] Position size: confidence=${entryConfidence}%, scale=${confidenceScale.toFixed(2)}, ${scaledMaxPercent.toFixed(1)}% of $${effectivePortfolioValue.toFixed(0)} = $${maxPositionValue.toFixed(0)}`,
      { sessionId, sessionName: session.name, symbol }
    );

    if (sessionMaxPercent > ctx.GLOBAL_MAX_POSITION_PERCENT) {
      tradingLogger.logRisk('Position size capped', {
        sessionId,
        sessionName: session.name,
        symbol,
        reason: `${sessionMaxPercent}% -> ${effectiveMaxPercent}%`,
        value: sessionMaxPercent,
        threshold: ctx.GLOBAL_MAX_POSITION_PERCENT,
      });
    }
    ...
    try {
      ...
      /* use effectivePortfolioValue as fallback */
    }
    const totalExposurePercent =
      accountEquity > 0 ? (totalExposure / accountEquity) * 100 : 0;
    if (totalExposurePercent >= ctx.GLOBAL_MAX_TOTAL_EXPOSURE_PERCENT) {
      tradingLogger.logRisk('Total exposure cap', {
        sessionId,
        sessionName: session.name,
        symbol,
        reason: `${totalExposurePercent.toFixed(1)}% >= ${ctx.GLOBAL_MAX_TOTAL_EXPOSURE_PERCENT}% cap`,
        value: totalExposurePercent,
        threshold: ctx.GLOBAL_MAX_TOTAL_EXPOSURE_PERCENT,
        action: `Blocking ${symbol} entry`,
      });
      return;
    }
    // Reduce max position value if approaching total exposure limit
    const remainingCapacity =
      ((ctx.GLOBAL_MAX_TOTAL_EXPOSURE_PERCENT - totalExposurePercent) / 100) *
      effectivePortfolioValue;
    ...
    if (
      ...
      tradingLogger.logRisk('Position size capped by maxPositionSize', {
        sessionId,
        sessionName: session.name,
        symbol,
        reason: `$${maxPositionValue.toFixed(0)} -> $${maxPositionSizeDollars}`,
        value: maxPositionValue,
        threshold: maxPositionSizeDollars,
      });
      maxPositionValue = maxPositionSizeDollars;
    }
    ...
    if (!maxPositionValue || isNaN(maxPositionValue) || maxPositionValue <= 0) {
      tradingLogger.logError(
        `[AI Engine] Invalid maxPositionValue for ${symbol}`,
        {
          sessionId,
          sessionName: session.name,
          symbol,
          error: `$${maxPositionValue}, portfolioValue=$${effectivePortfolioValue.toFixed(2)}, exposure=${totalExposurePercent.toFixed(1)}%`,
        }
      );
      return;
    }
    ...
    if (!currentPrice || currentPrice <= 0) {
      tradingLogger.logError(`[AI Engine] Invalid price for ${symbol}`, {
        ...
        sessionName: session.name,
        symbol,
        error: `price=${decision.currentPrice}`,
      });
      return;
    }
    ...
    if (isCrypto) {
      ...
      if (quantity < minQuantity) {
        tradingLogger.logInfo(
          `Position too small for ${symbol}: $${(quantity * currentPrice).toFixed(2)} (min $10)`,
          { sessionId, sessionName: session.name, symbol }
        );
        return;
      }
    ...
    tradingLogger.logInfo(
      `[AI Engine] Calculated position: ${qtyDisplay} ${isCrypto ? 'units' : 'shares'} of ${symbol} @ $${currentPrice.toFixed(2)} (max: $${maxPositionValue.toFixed(2)}, ${tradingMode.toUpperCase()})`,
      { sessionId, sessionName: session.name, symbol }
    );
    ...
    try {
      ...
      if (orderValue > availableBuyingPower) {
        tradingLogger.logRisk('Insufficient buying power', {
          sessionId,
          sessionName: session.name,
          symbol,
          reason: `Need $${orderValue.toFixed(2)}, have $${availableBuyingPower.toFixed(2)}`,
          value: orderValue,
          threshold: availableBuyingPower,
        ...
        if (
          maxAffordableQty < 1 ||
          (isCrypto && maxAffordableQty * currentPrice < 10)
        ) {
          tradingLogger.logInfo(
            `[AI Engine] Cannot afford minimum position for ${symbol}, skipping`,
            { sessionId, sessionName: session.name, symbol }
          );
          return;
        }
        ...
        tradingLogger.logInfo(
          `[AI Engine] Reduced position to ${quantity} ${isCrypto ? 'units' : 'shares'} to fit buying power`,
          { sessionId, sessionName: session.name, symbol }
        );
      }
    } catch (bpError) {
      tradingLogger.logError('[AI Engine] Buying power check failed', {
        sessionId,
        sessionName: session.name,
        symbol,
        error: `${bpError.message} - skipping entry`,
      });
      return;
    }
    ...
    if (existingPending) {
      ...
      if (elapsed < ctx.PENDING_ORDER_TIMEOUT_MS) {
        tradingLogger.logRisk('Duplicate order blocked', {
          sessionId,
          sessionName: session.name,
          symbol,
          reason: `Order ${existingPending.orderId.slice(0, 8)} pending (${(elapsed / 1000).toFixed(1)}s ago)`,
        });
        return;
      }
    ...
    tradingLogger.logExecution('BUY_ORDER', symbol, {
      quantity,
      price: decision.currentPrice,
      sessionId,
      sessionName: session.name,
      reason: `Market order (${tradingMode.toUpperCase()})`,
    });
    ...
    ctx.entryContexts.get(sessionId).set(symbol, {
      ...
      tradingMode,
      // Strategy plugin that generated this entry — for per-source P&L attribution.
...
async function executeExit(sessionId, symbol, decision) {
  ...
  if (!exitCheck.allowed) {
    tradingLogger.logInfo(
      `[AI Engine] Exit for ${symbol} blocked — in-flight by another session; will retry next tick`,
      { sessionId, sessionName: session?.name, symbol }
    );
    return;
  }
  ...
  try {
    ...
    const isStopLoss =
      decision.exitReason?.toLowerCase().includes('stop loss') ||
      decision.reason?.toLowerCase().includes('stop loss') ||
      (decision.reasons &&
        decision.reasons.some(f => f.toLowerCase().includes('stop loss')));

    // Determine if this is an end-of-day exit
    const isEodExit =
      decision.exitReason?.toLowerCase().includes('end-of-day') ||
    ...
    const isEmergencyExit =
      (isStopLoss && allowStopLossExit) || (isEodExit && allowEodExit);
    ...
    if (isEmergencyExit && !session.config.autoTrade) {
      ...
      tradingLogger.logRisk(`EMERGENCY ${exitType}`, {
        sessionId,
        sessionName: session.name,
        symbol,
        reason: `${exitType} executing while autoTrade is off`,
        action: 'Force selling',
      });
      websocketServer.sendAlert(session.userId, {
        type: 'error',
    ...
    // SIMULATION MODE: route through the simulated executor for real P&L tracking.
    if (session.config.simulationMode) {
      return simulatedExecutor.simulatedExit(session, symbol, decision);
    }
    ...
    try {
      ...
      if (!quantity || quantity <= 0) {
        ...
        try {
          ...
          quantity =
            parseFloat(alpacaPosition.qty) ||
            parseFloat(alpacaPosition.quantity);
        } catch (e) {
          ...
          if (errorMsg.includes('position') && errorMsg.includes('not found')) {
            ...
            tradingLogger.logError(
              `[AI Engine] Could not get position for ${normalizedSymbol}`,
              {
                sessionId,
                sessionName: session.name,
      ...
      if (!quantity || quantity <= 0) {
        tradingLogger.logError('[AI Engine] No valid quantity to sell', {
          sessionId,
          sessionName: session.name,
          symbol: normalizedSymbol,
        });
      ...
      if (isPartialExit) {
        ...
        tradingLogger.logInfo(
          `[AI Engine] Partial exit: selling ${quantity} of ${originalQuantity} shares (${(partialPercent * 100).toFixed(0)}%)`,
          { sessionId, sessionName: session.name, symbol }
        );
      }
      ...
      try {
        const openOrders = await alpacaClient.getOrders(
          { status: 'open', symbols: normalizedSymbol },
          tradingMode
        );
        const pendingForSymbol = (openOrders || []).filter(
          o => o.symbol === normalizedSymbol
        );
        for (const pendingOrder of pendingForSymbol) {
          await alpacaClient.cancelOrder(pendingOrder.id);
          tradingLogger.logInfo(
            `[AI Engine] Cancelled pending ${pendingOrder.side} order for ${normalizedSymbol} to free shares for exit`,
            {
              sessionId,
              sessionName: session.name,
              symbol: normalizedSymbol,
        ...
        tradingLogger.logInfo(
          `[AI Engine] Could not cancel pending orders for ${normalizedSymbol}: ${e.message}`,
          { sessionId, sessionName: session.name, symbol: normalizedSymbol }
        );
      }
      ...
      try {
        const alpacaPos = await alpacaClient.getPosition(
          normalizedSymbol,
          tradingMode
        );
        brokerQty =
          parseFloat(alpacaPos.qty) || parseFloat(alpacaPos.quantity) || 0;
      } catch (e) {
      ...
      const outsideRegularHours =
        !ctx.isMarketOpen() && ctx.isExtendedHoursOpen();
      ...
      if (quantity < brokerQty && quantity > 0) {
        ...
        tradingLogger.logInfo(
          `[AI Engine] Partial sell: session owns ${quantity} of ${brokerQty} total ${normalizedSymbol} shares`,
          { sessionId, sessionName: session.name, symbol: normalizedSymbol }
        );
        ...
        tradingLogger.logInfo(
          `[AI Engine] Extended-hours full close: selling ${quantity} ${normalizedSymbol}`,
          { sessionId, sessionName: session.name, symbol: normalizedSymbol }
        );
        result = await ctx.placeOrderForAsset(
          {
        ...
        result = await ctx.closePositionForAsset(
          normalizedSymbol,
          tradingMode,
          assetType
        );
      ...
      tradingLogger.logExecution('SELL_ORDER', symbol, {
        quantity,
        price: decision.currentPrice,
        sessionId,
        sessionName: session.name,
        reason: `Exit order (${tradingMode.toUpperCase()})`,
      });
      ...
      if (result.id) {
        for (let attempt = 0; attempt < 10; attempt++) {
          await new Promise(resolve => setTimeout(resolve, 500)); // wait 500ms
          try {
            filledOrder = await alpacaClient.getOrderById(
              result.id,
              tradingMode
            );
            if (filledOrder.status === 'filled' && filledOrder.filledAvgPrice) {
      ...
      if (isPartialExit) {
        ...
        const partialPnl =
          entryPrice > 0 ? (filledPrice - entryPrice) * quantity : 0;

        tradingLogger.logInfo(
          `[AI Engine] Partial exit completed: ${quantity} of ${originalQuantity} ${symbol} @ $${filledPrice.toFixed(2)} (P&L: $${partialPnl.toFixed(2)})`,
          { sessionId, sessionName: session.name, symbol }
        );
      ...
      tradingLogger.logInfo(
        `[AI Engine] ${symbol}: Cooldown started (${ctx.TRADE_COOLDOWN_MINUTES} min)`,
        { sessionId, sessionName: session.name, symbol }
      );
      ...
      const entryContext =
        ctx.entryContexts.get(sessionId)?.get(symbol) || null;
      ...
      tradingLogger.logExecution('SELL', symbol, {
        ...
        pnlPercent:
          entryPrice > 0
            ? ((filledPrice - entryPrice) / entryPrice) * 100
            : decision.pnlPercent,
      });
      ...
      session.stats.winRate =
        session.stats.totalTrades > 0
          ? parseFloat(
              ((session.stats.wins / session.stats.totalTrades) * 100).toFixed(
                1
              )
            )
          : 0;
      ...
      ctx.addSessionTrade(session.sessionId, {
        ...
        totalValue: quantity * filledPrice,
        pnl,
        // Attribute realized P&L to the source that opened the position.
        source: entryContext?.source || 'unknown',
      });
      ...
      if (
        error.message &&
        error.message.includes('insufficient qty available')
      ) {
        tradingLogger.logInfo(
          `[AI Engine] Clearing ghost position ${symbol} from session portfolio (Alpaca confirms 0 qty)`,
          { sessionId, sessionName: session.name, symbol }
        );
